...
public class CardDisplayController : MonoBehaviour, IPointerEnterHandler, IPointerExitHandler {
    ...
    public void Start() {
        ...
        if (isCombatCard) {
            ...
            GetComponent<Draggable>().onDragStart += (drag, drop) => {
                if (GameManager.Instance.Phase == Enums.GameplayPhase.Planning)
                    for (int i = 0; i < GameManager.Instance.Party().Count; i++) {
                        if (GameManager.Instance.Party(i).data.PrimaryAffinity == cardData.Affinity 
                            || GameManager.Instance.Party(i).data.SecondaryAffinity == cardData.Affinity)
                            if (!GameManager.Instance.Party(i).Incapacitated || !GameManager.Instance.Party(i).Defeated)
                            GameManager.Instance.Party(i).toggleHighlight(true);
                    }
            };
            GetComponent<Draggable>().onDragStop += (drag, drop) => {
                for (int i = 0; i < GameManager.Instance.Party().Count; i++) {
                    GameManager.Instance.Party(i).toggleHighlight(false);
                }
